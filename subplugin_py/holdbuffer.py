...
class Holdbuffer(GstBase.BaseTransform):
    GST_PLUGIN_NAME = "holdbuffer"
    ...
    def __init__(self):
        super(Holdbuffer, self).__init__()
        print("Initialized Holdbuffer plugin")
    ...
    def do_transform_ip(self, buf):
        ...
        if len(self.accumulated_data) >= self.threshold_size:
            ...
            # new_memory= Gst.Memory.new_wrapped(0, b'', 0, 0, None, None)
            # buf.replace_all_memory(new_memory)
            ...
            # structure = Gst.Structure.new_empty("pause-pipeline")
            # message = Gst.Message.new_application(self.get_parent(), structure)
            # bus = self.get_pipeline_bus()
            # if bus:
            #     bus.post(message)
            # return Gst.FlowReturn.OK
...
GObject.type_register(Holdbuffer)
__gstelementfactory__ = (Holdbuffer.GST_PLUGIN_NAME, Gst.Rank.NONE, Holdbuffer)
