...
gst_splitaddheadc_chain(GstPad *pad, GstObject *parent, GstBuffer *buf)
{
  ...
  guint indx;
  guint8 pack_accumulate_frame_number = (guint8)filter->accumulate_frame_number;
  for (indx = 1; indx <= filter->accumulate_frame_number; indx++)
  {
    guint8 pack_indx = (guint8)indx;
    g_print("%u\n", indx);

    /* Compute the segment of data */
    gsize offset = (indx - 1) * filter->single_frame_size;
    gsize segment_size = filter->single_frame_size;
    ...
    guint8 *new_data = g_malloc(new_single_frame_size);
    if (!new_data)
    {
    ...
    new_data[0] = pack_accumulate_frame_number;
    new_data[1] = pack_indx;
    memcpy(new_data + 2, pack_data_segment, filter->single_frame_size);

    /* Create a new buffer */
    GstBuffer *new_buffer = gst_buffer_new_allocate(NULL, new_single_frame_size, NULL);
    if (!new_buffer)
    {
    ...
    memcpy(map_info_new.data, new_data, new_single_frame_size);
